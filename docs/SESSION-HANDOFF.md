...
> 마지막 업데이트: 2026-06-30 3차 (Round 24-A 배포 완료)
> 다음 세션은 **이 파일을 가장 먼저** 읽고 시작한다.

## 현재 위치

**Round 24-A(Privacy 광고 고지 + Terms 정정 + design-preview 프로덕션 제외) 단독 배포 완료. 빌드·로컬·라이브 검증 전 항목 PASS. 다음 단계는 R24-B 사전 감사(ads.txt + AdSense 사이트 연결 스크립트). 광고 슬롯·CMP·Consent Mode는 아직 미구현.**

## 마지막 커밋 (origin/main · 로컬 HEAD · 서버 라이브 HEAD 모두 동일)

`be8d412` — Round 24-A: update privacy terms and hide design preview
...
- seoulautonomous.com 라이브 (Docker + Caddy), 서버 HEAD `be8d412`
- 새 이미지 `5b647441350e` (sha256 `5b647441…`) / 롤백 태그 `seoul-autonomous-web:rollback-c5b8abc`(=`47a89e955939`) 박제, `rollback-0a27b2c` 보존
...
2. **다음 단계: R24-B 사전 감사** — ads.txt + AdSense 사이트 연결 스크립트(`<head>`). publisher ID는 `<CONFIRMED_PUBLISHER_ID>`로만 표기, 실값 노출 금지. 연결 스크립트 ≠ 슬롯. Round 22/R24-A와 혼합 금지
3. R24-C/D(광고 슬롯·자동광고·CMP·Consent Mode)는 별도 라운드 분리. Round 23(GA4 커스텀 이벤트)도 별도
...
- **롤백**: 배포는 `:latest`를 덮음 → 배포 전 현재 이미지를 `rollback-<해시>` 태그로 박제할 것. 현재 직전: `rollback-c5b8abc`(=`47a89e955939`), 그 전: `rollback-0a27b2c`.
...
2. [ ] 최신 날짜별 핸드오프 `docs/handoff/HANDOFF-20260630_3.md` 읽기
3. [ ] MEMORY.md 확인
4. [ ] 라이브 확인: https://seoulautonomous.com/ko/night-bus-map , /ko/privacy , /ko/terms
