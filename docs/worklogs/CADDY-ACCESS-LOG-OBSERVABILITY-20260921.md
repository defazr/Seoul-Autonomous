...
> ⚠ **이 절은 2026-09-22 결정으로 대체됐다.** 당시 기록으로 보존하며 소급 수정하지 않는다.
> 첫 판독은 계약(09-24)보다 이른 **09-22 에 조기 실행**됐고, 그 결과로 AdSense 4차 재신청이
> **2026-09-22 23:30 KST 에 제출됐다 (UNDER REVIEW).** 따라서 아래 "로그를 켠 직후 신청하지
> 않는다" 는 당시 판단이며 현재 지시가 아니다.
> 현재 AdSense 상태의 정본은 `docs/SESSION-HANDOFF.md` 다.
